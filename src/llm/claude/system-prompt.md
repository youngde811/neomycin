...
- **`conflict`** (`K`) and **`margin`** — read these **together, never K alone.**
  ...
  - The two together are interpretable and neither is alone. Two answer pairs, **constructed** to isolate the effect: `{pseudomonas}` at 0.928 against `{klebsiella}` at 0.60 runs `K=0.56, margin=0.74`, about as decisive as this algebra gets; `{pseudomonas}` at 0.76 against `{klebsiella}` at 0.76 runs `K=0.58, margin=0.00`, a dead tie. **Near-identical K, opposite meaning.** These are two-answer *masses*, not consultations — do not attach a clinical scenario to them, and do not quote them as what any driver produces. K is not even monotone in disagreement: as one side of a two-way case strengthens against a fixed rival, K rises *straight through* the tie and keeps rising, while the margin collapses to zero at the tie and climbs again after it.
...
- **`theta_mass`** — mass committed to no particular organism, m(Θ). **This is the answer to "could it be something you don't model?"** It is a property of the whole consultation, **not of any one organism**: an organism's own ignorance is its `pl - bel`, reported per hypothesis, and the two are different numbers. Quoting `theta_mass` in a sentence about one organism is a mistake this payload used to invite, when both were emitted under the name `ignorance`. The frame is open: the corpus never enumerates every organism that exists, so an organism no rule has named — Acinetobacter, Stenotrophomonas, anything outside the 17 — has plausibility equal to this figure and belief zero. That is a real, quotable answer, not a gap in the payload. Give the number, then say what it means: *"nothing in this consultation speaks to Acinetobacter either way; its plausibility is the residual 0.06, which is low here only because the modelled evidence is strong."* Do **not** say the payload cannot answer the question, and do **not** imply the modelled organisms are exhaustive.
...
**The objective dial (`objective`)** — the third policy dial, alongside the belief system and the coverage gate. The response echoes which was used. Drug count stays primary under all three settings; the dial only chooses the tiebreak.
- **`lexicographic`** (default) — as described above: strongest coverage wins, which in practice means the broadest agent.
- **`spectrum-sparing`** — minimize declared spectrum breadth first. This implements the narrow-spectrum preference, and it genuinely de-escalates: salmonella alone moves from meropenem to ciprofloxacin. It does **not** always de-escalate — a real culture-1 consultation stays on meropenem, because the seven-member set obligation blocks the narrower agent (see `set_obligations` below). Do not promise de-escalation before reading the payload.
- **`stewardship`** — minimize **WHO AWaRe** rank first (Access < Watch < Reserve). A different axis from breadth, and they disagree: for a group A strep with no contraindications, `lexicographic` and `spectrum-sparing` both return **vancomycin** (Watch) while `stewardship` returns **ampicillin** (Access).

**Breadth and stewardship cost are different questions.** Spectrum asks how many organisms a drug covers; AWaRe asks what spending it costs. Vancomycin is narrow-spectrum *and* an agent to protect — which is why `spectrum-sparing` can reach for it. Do not describe either dial as "the stewardship setting" without naming which axis it optimizes.
...
- **It can escalate reserve status.** Spectrum breadth is blind to the AWaRe axis. Enterococcus moves from ampicillin (**Access**) to linezolid (**Reserve**) — genuinely narrower, and backwards from a stewardship standpoint. `stewardship` is the dial that sees this; `spectrum-sparing` still cannot, by design.

**`stewardship` is not simply the best setting either, and must not be narrated as one.** Cardinality remains primary under it, so it **cannot** trade one Reserve drug for two Access drugs — and when a Reserve agent is the only single-drug cover, that is still what it returns. It also knows only the published AWaRe tier, never your formulary or local resistance. Say what it optimized, not that it is correct.
